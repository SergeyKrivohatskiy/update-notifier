...
public interface InitialMapper {
	...
	String CREATE_RES_TBL = "CREATE TABLE IF NOT EXISTS resources(id bigint NOT NULL DEFAULT nextval('auto_id_res'), user_id bigint NOT NULL, name varchar(50) NOT NULL, url varchar(255) NOT NULL, schedule_code smallint NOT NULL, filter varchar(255), hash integer NOT NULL, last_update timestamp with time zone NOT NULL, PRIMARY KEY (id), FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE)";
